...
if ( ! function_exists( 'ecommerce_gem_is_featured_slider_type_pages' ) ) :

	/**
	 * Check if featured slider is active and pages is selected for slider option.
	 *
	 * @since 1.0.0
	 *
	 * @param WP_Customize_Control $control WP_Customize_Control instance.
	 ...
	 * @return bool Whether the control is active to the current preview.
	 */
	function ecommerce_gem_is_featured_slider_type_pages( $control ) {

		if ( true == $control->manager->get_setting( 'theme_options[slider_status]' )->value() &&  'pages' == $control->manager->get_setting( 'theme_options[slider_type]' )->value() ) {
			return true;
		} else {
			return false;
		}
...
if ( ! function_exists( 'ecommerce_gem_is_featured_slider_type_shortcode' ) ) :

	/**
	 * Check if featured slider is active and shortcode is selected for slider option.
	 *
	 * @since 1.0.0
	 *
	 * @param WP_Customize_Control $control WP_Customize_Control instance.
	 ...
	 * @return bool Whether the control is active to the current preview.
	 */
	function ecommerce_gem_is_featured_slider_type_shortcode( $control ) {

		if ( true == $control->manager->get_setting( 'theme_options[slider_status]' )->value() &&  'shortcode' == $control->manager->get_setting( 'theme_options[slider_type]' )->value() ) {
			return true;
		} else {
			return false;
		}
