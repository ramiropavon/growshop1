...
if ( ! class_exists( 'eCommerce_Gem_About' ) ) {
	...
	class eCommerce_Gem_About {
		...
		public function setup_config() {
			...
			$this->logo_link     = isset( $this->config['logo_link'] ) ? $this->config['logo_link'] : 'https://www.prodesigns.com/';
			$this->tabs          = isset( $this->config['tabs'] ) ? $this->config['tabs'] : array();
			$this->notification  = isset( $this->config['notification'] ) ? $this->config['notification'] : ( '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our %2$swelcome page%3$s.', 'ecommerce-gem-plus' ), $this->theme_name, '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '">', '</a>' ) . '</p><p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '" class="button button-primary" style="text-decoration: none;">' . sprintf( esc_html__( 'Get started with %s', 'ecommerce-gem-plus' ), $this->theme_name ) . '</a></p>' );
		}
		...
		public function setup_actions() {
			...
			add_action( 'wp_ajax_pt_about_action_dismiss_recommended_action', array( $this, 'dismiss_recommended_action_callback' ) );
			add_action( 'wp_ajax_nopriv_pt_about_action_dismiss_recommended_action', array( $this, 'dismiss_recommended_action_callback' ) );
		}
		...
		public function load_assets( $hook ) {
			...
			if ( 'appearance_page_' . $this->page_slug === $hook ) {
				...
				wp_enqueue_style( 'ecommerce-gem-about', get_template_directory_uri() . '/includes/theme-info/css/about.css', array(), '2.1.2' );
				wp_enqueue_script( 'ecommerce-gem-about', get_template_directory_uri() . '/includes/theme-info/js/about.js', array( 'jquery' ), '2.1.2' );
				$js_vars = array(
					'ajaxurl' => esc_url( admin_url( 'admin-ajax.php' ) ),
				);
				wp_localize_script( 'ecommerce-gem-about', 'eCommerceGemAboutObject', $js_vars );
			}
		}
						...
						if ( ! empty( $action_value['plugin_slug'] ) ) {
							...
							switch ( $active['needs'] ) {
								...
								case 'install':
									...
									$label = esc_html__( 'Install', 'ecommerce-gem-plus' );
									break;
								case 'activate':
									$class = 'activate-now button button-primary';
									$label = esc_html__( 'Activate', 'ecommerce-gem-plus' );
									break;
								case 'deactivate':
									$class = 'deactivate-now button';
									$label = esc_html__( 'Deactivate', 'ecommerce-gem-plus' );
									break;
							}
							?>
		...
		public function upgrade_to_pro() {
			...
			if ( isset( $upgrade_to_pro['button_link'] ) && ! empty( $upgrade_to_pro['button_link'] ) ) {
				$button_text = esc_html__( 'Upgrade to Pro', 'ecommerce-gem-plus' );
		...
		public function useful_plugins() {
			...
			if ( isset( $tgmpa ) ) {
				$useful_plugins = ( isset( $this->config['useful_plugins'] ) ) ? $this->config['useful_plugins'] : array();
				$url            = $tgmpa->get_tgmpa_url();
				$complete       = $tgmpa->is_tgmpa_complete();
				$all_plugins    = $tgmpa->plugins;

				if ( true === $complete ) {
					echo '<div class="all-active"><span class="dashicons dashicons-yes"></span>&nbsp;' . esc_html__( 'Great, all recommended plugins are intalled and active.', 'ecommerce-gem-plus' ) . '</div>';
				}

				if ( isset( $useful_plugins['description'] ) && ! empty( $useful_plugins['description'] ) ) {
					echo '<div>' . wp_kses_post( $useful_plugins['description'] ) . '</div>';
				}

				if ( true !== $complete ) {
					echo '<div class="button-wrap"><a href="' . esc_url( $url ) . '" class="button button-primary">' . esc_html__( 'Manage Plugins', 'ecommerce-gem-plus' ) . '</a></div>';
				}

				if ( ! empty( $all_plugins ) ) {
					if ( isset( $useful_plugins['plugins_list_title'] ) && ! empty( $useful_plugins['plugins_list_title'] ) ) {
						echo '<h4>' . esc_html( $useful_plugins['plugins_list_title'] ) . '</h4>';
					}
		...
		public function free_pro() {
		    ...
		    if ( ! empty( $free_pro ) ) {
		        ...
		        if ( ! empty( $free_pro ) && is_array( $free_pro ) ) {
		            ...
		            echo '<tr>';
		            echo '<th></th>';
		            echo '<th>' . esc_html__( 'eCommerce Gem','ecommerce-gem-plus' ) . '</th>';
		            echo '<th>' . esc_html__( 'eCommerce Gem Plus','ecommerce-gem-plus' ) . '</th>';
		            echo '</tr>';
		            echo '</thead>';
		            echo '<tbody>';
		            ...
		            echo '<td></td>';
		            echo '<td colspan="2"><a href="https://www.prodesigns.com/wordpress-themes/downloads/ecommerce-gem-plus/" target="_blank" class="button button-primary button-hero">eCommerce Gem Plus</a></td>';
		            echo '</tr>';
